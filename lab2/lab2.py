...
def extract_dataset(archive_path=DATASET_ARCHIVE_PATH, extract_path=DATASET_EXTRACT_PATH):
...
def load_images(root_dir):
    ...
    for alphabet_dir in sorted([d for d in os.listdir(root_dir) if os.path.isdir(os.path.join(root_dir, d))]):
...
def split_dataset(data, targets, test_size=0.2):
    ...
    return data[train_indices], targets[train_indices], data[val_indices], targets[val_indices]
...
def prepare_omniglot_dataset():
    ...
    if USE_SEPARATE_DATASETS:
        ...
        print("Using separate datasets for training (background) and validation (evaluation)")
        ...
        print("Mixing datasets (background and evaluation) and splitting into 80-20 for training-validation")
